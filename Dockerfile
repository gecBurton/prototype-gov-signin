...
# false here, which is when ALLOWED_HOSTS becomes required); collectstatic
# never opens a database connection, serves a request, or talks to Hydra, so
# the dummy values are fine.
