...
def _make_app(fake_hydra, team, **kwargs):
    kwargs.setdefault("name", "Catalogue App")
...
def test_directory_visibility(
    client, fake_hydra, directory_team, is_active, listed, visible
):
    _make_app(
        fake_hydra,
        directory_team,
...
def test_directory_shows_name_description_and_link(client, fake_hydra, directory_team):
    _make_app(
        fake_hydra,
        directory_team,
        name="Grafana",
...
def test_directory_is_global_not_scoped_to_the_users_teams(
    client, fake_hydra, directory_team
):
    # The directory is a catalogue of what exists: a user sees listed apps even
    # from teams they do not belong to.
    _make_app(fake_hydra, directory_team, name="Someone Elses App")
...
def test_user_with_access_sees_no_badge(client, fake_hydra, directory_team):
    directory_team.allowed_email_domains.create(domain="example.com")
    _make_app(fake_hydra, directory_team, name="Insider App")
...
def test_ineligible_user_sees_no_access_badge_but_app_still_listed(
    ...
    directory_team.allowed_email_domains.create(domain="example.com")
    _make_app(fake_hydra, directory_team, name="Restricted App")
...
def test_directory_badge_ignores_additional_emails(client, fake_hydra, directory_team):
    # The directory access tag is computed from team domains only. The
    # per-app additional_emails allowlist is still honoured at the login-
    # challenge view (so this user CAN sign in), but is not reflected in the
    # badge.
    _make_app(
        fake_hydra,
        directory_team,
        name="VIP App",
        additional_emails=["vip@other.org"],
    )
...
def test_directory_paginates_at_twenty_per_page(client, fake_hydra, directory_team):
    for n in range(25):
        _make_app(fake_hydra, directory_team, name=f"Paginated App {n:02d}")
    _signed_in(client)
...
def test_directory_is_not_paginated_below_the_page_size(
    client, fake_hydra, directory_team
):
    _make_app(fake_hydra, directory_team, name="Only App")
    _signed_in(client)
...
def test_pagination_preserves_access_badging_on_later_pages(
    client, fake_hydra, directory_team
):
    directory_team.allowed_email_domains.create(domain="example.com")
    for n in range(25):
        _make_app(fake_hydra, directory_team, name=f"App {n:02d}")
...
def test_search_matches_name(client, fake_hydra, directory_team):
    _make_app(fake_hydra, directory_team, name="Payments")
    _make_app(fake_hydra, directory_team, name="Notifications")
    _signed_in(client)
...
def test_search_matches_description(client, fake_hydra, directory_team):
    _make_app(fake_hydra, directory_team, name="Alpha", description="handles invoices")
    _make_app(fake_hydra, directory_team, name="Beta", description="sends letters")
    _signed_in(client)
...
def test_search_is_case_insensitive(client, fake_hydra, directory_team):
    _make_app(fake_hydra, directory_team, name="Grafana")
    _signed_in(client)

    assert b"Grafana" in client.get(URL, {"search": "GRAF"}).content


def test_access_only_hides_apps_without_access(client, fake_hydra, directory_team):
    directory_team.allowed_email_domains.create(domain="example.com")
    _make_app(fake_hydra, directory_team, name="Insider App")
    other = Team.objects.create(name="Other Team")
    other.allowed_email_domains.create(domain="elsewhere.org")
    _make_app(fake_hydra, other, name="Outsider App")
    _signed_in(client, email="insider@example.com")
...
def test_search_and_access_only_combine(client, fake_hydra, directory_team):
    directory_team.allowed_email_domains.create(domain="example.com")
    _make_app(
        fake_hydra, directory_team, name="Payments service"
    )  # matches and accessible
    other = Team.objects.create(name="Other Team")
    other.allowed_email_domains.create(domain="elsewhere.org")
    _make_app(fake_hydra, other, name="Payments portal")  # matches but no access
    _signed_in(client, email="insider@example.com")
...
def test_filtered_empty_state(client, fake_hydra, directory_team):
    _make_app(fake_hydra, directory_team, name="Grafana")
    _signed_in(client)
...
def test_search_form_renders_controls(client, fake_hydra, directory_team):
    _signed_in(client)
    content = client.get(URL).content.decode()
...
def test_access_only_checkbox_stays_checked_after_filtering(
    client, fake_hydra, directory_team
):
    _signed_in(client)
    content = client.get(URL, {"access_only": "1"}).content.decode()
