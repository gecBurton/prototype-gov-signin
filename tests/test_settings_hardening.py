...
def _import_settings(env_overrides):
    ...
    env = {
        ...
        "PYTHONPATH": str(IAM_DIR),
        "SECRET_KEY": "settings-test-key",
        # Satisfy the database guard by default so each test isolates the one
        # guard it targets; django.setup() configures settings without
        # opening a connection or calling Hydra.
